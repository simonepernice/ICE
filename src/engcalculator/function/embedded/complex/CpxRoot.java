 ...
 */
public final class CpxRoot extends FunctionPrefixByExpression {

    private static final String EXP = "abs(c)^(1/n)*exp(I*(complexArg(c)+(1 .. n)*2*PI)/n)";
    private static final String HELP = "... (complexNumber, n) computes the n-th roots of the given complexNumber.\nIt provides all the solutions available. It uses the polar form to compute all the roots which may increase the result of the ending intervals.";
    ...
    private final static String[] RESULT = {"-1, 1"};
    
    
    public CpxRoot () {
        super("complex", "Root", (byte) 2, DOMAIN, false, HELP, EXAMPLE, RESULT, EXP, true, VARS, GROUPS);
    }
    ...
    public List<MeasurementUnit> _computeMeasurementUnit(ListIntervals input) throws Exception {
        ...
        res.add(new MeasurementUnitAccumulator(input.getFirst().getMeasurementUnit()).div((float)input.getLast().getValue()).toMeasurementUnit());
        return res;
    }
