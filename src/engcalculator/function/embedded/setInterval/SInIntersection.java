 ...
 */
public final class SInIntersection extends FunctionInfixDualInputSingleOutput {
    private final static DomainList DOMAIN = new DomainList (new DomainIntervalReal());
    private final static String HELP = "intervalsA ... intervalsB returns the intersection between the two intervals A and B considered as set of real number.\nIf the two intervals have no intersection it returns an empty interval.";
    private final static String[] EXAMPLE_LEFT = {"6_7","5_7", "1_2", "3_4", "'to check if a list of adjacent intervals do not have any intersection the result has to be all -1';a=(1_2,2.1_5,9_10,11_12);intervalDirection(diffuseSpread('&',a))"};
    private final static String[] EXAMPLE_RIGHT = {"4_5","4_6", "2_4", "-3_-2",""};
    private final static String[] RESULT = {"6_5","5_6","2","3_-2","-1,-1,-1"};

    public SInIntersection() {
    ...
    public Interval _computeIntervals (Interval ls, Interval rs) throws Exception {
        return new IntervalReal (Math.max(ls.getLeft(), rs.getLeft()), Math.min(ls.getRight(), rs.getRight()));
    }
