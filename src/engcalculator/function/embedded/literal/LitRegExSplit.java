 ...
 */
public final class LitRegExSplit extends FunctionPrefix {
    private final static DomainList DOMAIN = new DomainList (new DomainIntervalLiteral(), new DomainIntervalLiteral());

    private final static String HELP = "... ('regular expression', literal) looks for regular expression within the literal and returns the literals without them.\nIt is possible to apply to several literals. For regular expression syntax see literalRegularExpressionMatch help.";
    private final static String[] EXAMPLE = {"',', 'ironman,hulk,spiderman,superman,mr.incredible'"};
    private final static String[] RESULT = {"'ironman','hulk','spiderman','superman','mr.incredible'"};

    public LitRegExSplit() {        
        super("literal", "RegularExpressionSplit", (byte) -2, DOMAIN, false, true, HELP, EXAMPLE, RESULT);
    }
    ...
    public ListIntervals _compute(ListIntervals input) throws Exception {
        ...
        for (int i=1; i< s; ++i) {
            String[] splits = p.split(input.get(i).getName());
            for (String st : splits)
                result.add(new IntervalLiteral(st));
        }                
        return result;
    }
