...
/**
 * A root finder for polynomials with real coefficients. It uses the
 * Jenkins-Traub algorithm to find the zeros of a polynomial.
 * 
* <p>
 * Reference:
 * <a href="http://en.wikipedia.org/wiki/Jenkins%E2%80%93Traub_algorithm">Wikipedia</a>.
 * 
 */

package engcalculator.function.embedded.polynomial;

import engcalculator.domain.DomainIntervalPointNotComplex;
 ...
 */
public final class PlySolveJT extends FunctionPrefix {

    private final static String HELP = "... (an, an-1, .., a1, a0) given the coefficients of polynomial it tries to find iterativaly all the available roots at the same time.\nThe algorithm is the one written by Jenkins Traub. This is a Java port of Fortran source code by Michael A. Jenkins. Copyright notice for the Java port: Copyright 2013 Christian d'Heureuse, Inventec Informatik AG, Zurich, Switzerland. The source code are available on  http://svn.source-code.biz/dsp-java/trunk/src/main/java/biz/source_code/dsp/math/PolynomialRootFinderJenkinsTraub.java  and http://svn.source-code.biz/dsp-java/trunk/src/main/java/biz/source_code/dsp/math/Complex.java  ";
    private final static String[] EXAMPLE = {"1,5,6", "(1, -6) ***** (1, -3)"};
    private final static String[] RESULT = {"-2,-3", "3, 6"};
    private final static DomainList INTERVALDOMAIN = new DomainList(new DomainIntervalPointNotComplex()); 

    public PlySolveJT() {
        super("polynomial", "SolveJT", (byte) -2, INTERVALDOMAIN, false, true, HELP, EXAMPLE, RESULT);
    }

    @Override
    public ListIntervals _compute(ListIntervals c) throws Exception {
        double[] coeffs = new double[c.size()];
        {
            int j = 0;
            for (Interval i : c) {
                coeffs[j++] = i.getValue();
        ...
        Complex[] res = env.rpoly(coeffs);
        ListIntervals result = new ListIntervals();

        for (Complex r : res) {
            result.add(new IntervalComplex(new IntervalPoint(r.re()), new IntervalPoint(r.im())));
        }
