 ...
 */
public final class IntCeiling extends FunctionPrefixSingleInputSingleOutput {

    private final static String HELP = "... (a) returns the smallest (closest to negative infinity) double value that is greater than or equal to the argument a and is a mathematical integer.\nSee also floor.";    
    private final static String[] EXAMPLE = {"-10.7_10.7","10.1_-5.8", "10.99"};
    private final static String[] RESULT = {"-10_11", "11_-5", "11"};
    private final static DomainList DOMAIN = new DomainList (new DomainIntervalReal());

    public IntCeiling() {
        super("integer", "Ceiling", DOMAIN, true, true, HELP, EXAMPLE , RESULT);
    }
    
    @Override
    public Interval _computeIntervals (Interval input) throws Exception {
        if (input.isIntervalPoint()) {
            double a1 = input.getValue();
            return new IntervalPoint(Math.ceil(a1));
        }
        double a1 = input.getLeft(), a2 = input.getRight();
        return new IntervalReal(Math.ceil(a1), Math.ceil(a2));
    }
