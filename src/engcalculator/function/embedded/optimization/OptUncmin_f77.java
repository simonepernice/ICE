/*
OptUncmin_f77.java copyright claim:
 ...
 *
 */
public final class OptUncmin_f77 extends Object {
     ...
     *must be supplied.  (See OptUncmin_methods.java and UncminTest.java.)
     ...
     *@param minclass   A class that implements the OptUncmin_methods
     *                  interface (see the definition in
     *                  OptUncmin_methods.java).  See UncminTest_f77.java for an
     ...
     *
     */
    public static void optif0_f77(int n, double x[], OptUncmin_methods minclass,
        ...
        OptUncmin_f77.dfault_f77(n, x, typsiz, fscale, method, iexp, msg,
                ndigit, itnlim, iagflg, iahflg, dlt,
                gradtl, stepmx, steptl);

        OptUncmin_f77.optdrv_f77(n, x, minclass, typsiz, fscale, method,
     ...
     *
     */
    public static void optif9_f77(int n, double x[], OptUncmin_methods minclass,
...
// AND DIAGONAL OF "A" TO UDIAG
        ...
        OptUncmin_f77.choldc_f77(n, a, diagmx, tol, addmax);
...
// "A" NOW GUARANTEED SAFELY POSITIVE DEFINITE
            ...
            OptUncmin_f77.choldc_f77(n, a, 0.0, tol, addmax);
...
// SET FLAGS
    ...
    public static void dogdrv_f77(int n, double x[], double f[], double g[],
            ...
            double fpls[], OptUncmin_methods minclass,
...
// FIND NEW STEP BY DOUBLE DOGLEG ALGORITHM
            ...
            OptUncmin_f77.dogstp_f77(n, g, a, p, sx, rnwtln, dlt, nwtake, fstdog,
                    wrk1, wrk2, cln, eta, sc, stepmx);

// CHECK NEW POINT AND UPDATE TRUST REGION

            OptUncmin_f77.tregup_f77(n, x, f, g, a, minclass, sc, sx, nwtake, stepmx,
...
// CALCULATE DOUBLE DOGLEG CURVE (SSD)
                ...
                eta[1] = .2
                        + (.8 * alpha * alpha) / (-beta * OptBlas_f77.ddot_f77(n, g, 1, p, 1));
...
// WHICH HAS SCALED LENGTH DLT
                    ...
                    dot1 = OptBlas_f77.ddot_f77(n, v, 1, ssd, 1);
                    dot2 = OptBlas_f77.ddot_f77(n, v, 1, v, 1);
...
// SOLVE LX=B. (FORWARD SOLVE)
     ...
     *
     */
    public static void fstocd_f77(int n, double x[], OptUncmin_methods minclass,
...
// UNIT VECTOR, AND EVALUATE I-TH  COMPONENT OF GRADIENT.
     ...
     *
     */
    public static void fstofd_f77(int n, double xpls[], OptUncmin_methods minclass,
...
// gradient
     ...
     *
     */
    public static void grdchk_f77(int n, double x[], OptUncmin_methods minclass,
...
// ANALYTIC GRADIENT.
        ...
        OptUncmin_f77.fstofd_f77(n, x, minclass, f, gest, sx, rnf);
        ...
        for (i = 1; i <= n; i++) {
            ...
            if (Math.abs(g[i] - gest[i])
                    > Math.max(Math.abs(g[i]), gs) * analtl) {
                ker = 1;
            }
     ...
     *@param wrk2       Workspace
     */
    public static void heschk_f77(int n, double x[], OptUncmin_methods minclass,
...
// COMPUTE FINITE DIFFERENCE APPROXIMATION H TO THE HESSIAN.
        ...
        if (iagflg[1] == 1) {
            OptUncmin_f77.fstofd_f77(n, x, minclass, g, a, sx, rnf, wrk1);
        }

        if (iagflg[1] != 1) {
            OptUncmin_f77.sndofd_f77(n, x, minclass, f, a, sx, rnf, wrk1, wrk2);
        }
...
// APPROXIMATION.
        ...
        for (j = 1; j <= n; j++) {
            ...
            if (Math.abs(a[j][j] - udiag[j])
                    > Math.max(Math.abs(udiag[j]), hs) * analtl) {
                ker = 1;
            }
            ...
            for (i = j + 1; i <= n; i++) {
                ...
                if (Math.abs(a[i][j] - a[j][i])
                        > Math.max(Math.abs(a[i][j]), hs) * analtl) {
                    ker = 1;
                }
...
// FIND NEW STEP BY MORE-HEBDON ALGORITHM
            ...
            OptUncmin_f77.hookst_f77(n, g, a, udiag, p, sx, rnwtln, dlt,
                    amu, dltp, phi, phip0,
...
// SOLVE L*Y = (SX**2)*P
                ...
                OptUncmin_f77.forslv_f77(n, a, wrk0, wrk0);
                phip0[1] = -Math.pow(OptBlas_f77.dnrm2_f77(n, wrk0, 1), 2) / rnwtln;
                fstime[1] = false;
...
// FACTOR H=L(L+)
                ...
                OptUncmin_f77.choldc_f77(n, a, 0.0, Math.sqrt(epsm), addmax);
...
// SOLVE H*P = L(L+)*SC = -G
                ...
                OptUncmin_f77.lltslv_f77(n, a, sc, wrk0);
...
// NOTHING HERE.  H IS IN THE UPPER PART AND IN UDIAG, STILL INTACT
                ...
                phip = -Math.pow(OptBlas_f77.dnrm2_f77(n, wrk0, 1), 2) / stepln;
...
// FORWARD SOLVE, RESULT IN X
        ...
        OptUncmin_f77.forslv_f77(n, a, x, b);

// BACK SOLVE, RESULT IN X

        OptUncmin_f77.bakslv_f77(n, a, x, x);
...
// NEWTON STEP LONGER THAN MAXIMUM ALLOWED
            ...
            scl = stepmx[1] / sln;
            OptUncmin_f77.sclmul_f77(n, scl, p, p);
            sln = stepmx[1];

        }

        slp = OptBlas_f77.ddot_f77(n, g, 1, p, 1);
        rln = 0.0;
...
// ONLY ONE POSITIVE CRITICAL POINT, MUST BE MINIMUM
                            ...
                            tlmbda = (-b
                                    + OptBlas_f77.sign_f77(1.0, a) * Math.sqrt(disc)) / (3.0 * a);
...
// CHECK TRUST REGION RADIUS
     ...
     *
     */
    public static void optdrv_f77(int n, double x[], OptUncmin_methods minclass,
            double typsiz[], double fscale[], int method[],
            int iexp[], int msg[], int ndigit[], int itnlim[],
            int iagflg[], int iahflg[], double dlt[],
            double gradtl[], double stepmx[], double steptl[],
...
// INITIALIZATION
        ...
        OptUncmin_f77.optchk_f77(n, x, typsiz, sx, fscale, gradtl, itnlim, ndigit,
...
// GRADIENT, IF REQUESTED.
        ...
        if (iagflg[1] == 0) {
            ...
            OptUncmin_f77.fstofd_f77(n, x, minclass, f, g, sx, rnf);
            ...
            if ((msg[1] / 2) % 2 == 0) {
                ...
                OptUncmin_f77.grdchk_f77(n, x, minclass, f, g, typsiz,
        ...
        OptUncmin_f77.optstp_f77(n, x, f, g, wrk1, itncnt, icscmx,
...
// HESSIAN WILL BE OBTAINED BY SECANT UPDATES.  GET INITIAL HESSIAN.
                ...
                OptUncmin_f77.hsnint_f77(n, a, sx, method);
...
// ROUTINE minclass.hessian FILLS ONLY LOWER TRIANGULAR PART AND DIAGONAL OF A).
                ...
                if (iahflg[1] == 0) {
                    ...
                    if ((msg[1] / 4) % 2 == 1) {
                        ...
                        OptUncmin_f77.heschk_f77(n, x, minclass, f, g, a, typsiz,
...
// (IF IAGFLG .EQ. 1) OR SNDOFD (OTHERWISE).
            ...
            if ((msg[1] / 8) % 2 == 0) {
                OptUncmin_f77.result_f77(n, x, f, g, a,
                        p, itncnt, 1);
            }
...
// SECFAC.)
                ...
                if (iexp[1] != 1 || method[1] == 3) {
                    ...
                    OptUncmin_f77.chlhsn_f77(n, a, epsm, sx, udiag);
...
// SOLVE FOR NEWTON STEP:  AP=-G
                ...
                OptUncmin_f77.lltslv_f77(n, a, p, wrk1);
...
// OR TO CHOOSE XPLS BY A GLOBAL STRATEGY.
                ...
                if (method[1] == 1) {
                    ...
                    OptUncmin_f77.lnsrch_f77(n, x, f, g, p, xpls, fpls, minclass,
                            mxtake, iretcd, stepmx, steptl,
                            sx);

                } else if (method[1] == 2) {

                    OptUncmin_f77.dogdrv_f77(n, x, f, g, a, p, xpls, fpls, minclass,
                            sx, stepmx, steptl, dlt, iretcd,
                            mxtake, wrk0, wrk1, wrk2, wrk3);

                } else {

                    OptUncmin_f77.hookdr_f77(n, x, f, g, a, udiag, p, xpls, fpls,
...
// SET IAGFLG FOR CENTRAL DIFFERENCES
                    ...
                    OptUncmin_f77.fstocd_f77(n, x, minclass, sx, rnf, g);
...
// CENTRAL DIFFERENCE GRADIENT
                    ...
                    OptUncmin_f77.fstocd_f77(n, xpls, minclass, sx, rnf, gpls);

                } else if (iagflg[1] == 0) {

// FORWARD DIFFERENCE GRADIENT

                    OptUncmin_f77.fstofd_f77(n, xpls, minclass, fpls, gpls, sx, rnf);
...
// CHECK WHETHER STOPPING CRITERIA SATISFIED
                ...
                OptUncmin_f77.optstp_f77(n, xpls, fpls, gpls, x, itncnt, icscmx,
                        itrmcd, gradtl, steptl, sx, fscale,
                        itnlim, iretcd, mxtake, msg);
...
// EVALUATE HESSIAN AT XPLS
                    ...
                    if (iexp[1] != 0) {
                        ...
                        if (method[1] == 3) {
                            ...
                            OptUncmin_f77.secunf_f77(n, x, g, a, udiag, xpls, gpls, epsm,
                                    itncnt, rnf, iagflg, noupdt,
                                    wrk1, wrk2, wrk3);

                        } else {

                            OptUncmin_f77.secfac_f77(n, x, g, a, xpls, gpls, epsm, itncnt,
                        ...
                        if (iahflg[1] == 1) {
                            ...
                            if (iagflg[1] == 1) {
                                ...
                                OptUncmin_f77.fstofd_f77(n, xpls, minclass, gpls, a,
                                        sx, rnf, wrk1);

                            } else {

                                OptUncmin_f77.sndofd_f77(n, xpls, minclass, fpls, a,
                                        sx, rnf, wrk1, wrk2);
                    ...
                    if ((msg[1] / 16) % 2 == 1) {
                        ...
                        OptUncmin_f77.result_f77(n, xpls, fpls, gpls, a, p, itncnt, 1);
...
// PRINT RESULTS
        ...
        if ((msg[1] / 8) % 2 == 0) {
            ...
            OptUncmin_f77.result_f77(n, xpls, fpls, gpls, a, p, itncnt, 0);
...
// WHICH IS UPPER HESSENBERG
        ...
        for (ii = 1; ii <= km1; ii++) {
            ...
            if (u[i] == 0.0) {
                ...
                OptUncmin_f77.qraux1_f77(n, a, i);
                u[i] = u[i + 1];

            } else {

                OptUncmin_f77.qraux2_f77(n, a, i, u[i], -u[i + 1]);
...
// TO UPPER TRIANGULAR (R*)
        ...
        for (i = 1; i <= km1; i++) {
            ...
            if (a[i][i] == 0.0) {
                ...
                OptUncmin_f77.qraux2_f77(n, a, i, t1, t2);
...
// PRINT HESSIAN FROM ITERATION K
    ...
    public static void secfac_f77(int n, double x[], double g[],
        ...
        den1 = OptBlas_f77.ddot_f77(n, s, 1, y, 1);
        snorm2 = OptBlas_f77.dnrm2_f77(n, s, 1);
        ynrm2 = OptBlas_f77.dnrm2_f77(n, y, 1);

        if (den1 >= Math.sqrt(epsm) * snorm2 * ynrm2) {

            OptUncmin_f77.mvmltu_f77(n, a, s, u);
            den2 = OptBlas_f77.ddot_f77(n, u, 1, u, 1);
...
// W = L(L+)S = HS
            ...
            OptUncmin_f77.mvmltl_f77(n, a, u, w);
...
// FIND Q, (L+) SUCH THAT  Q(L+) = (L+) + U(W+)
                ...
                OptUncmin_f77.qrupdt_f77(n, a, u, w);
...
// LOWER TRIANGULAR PART AND DIAGONAL
        ...
        if (den1 >= Math.sqrt(epsm) * snorm2 * ynrm2) {
            ...
            OptUncmin_f77.mvmlts_f77(n, a, s, t);

            den2 = OptBlas_f77.ddot_f77(n, s, 1, t, 1);
...
// BFGS UPDATE
     ...
     *
     */
    public static void sndofd_f77(int n, double xpls[], OptUncmin_methods minclass,
            double fpls[], double a[][], double sx[],
...
// CALCULATE SUB-DIAGONAL ELEMENTS OF COLUMN
    ...
    public static void tregup_f77(int n, double x[], double f[], double g[],
            double a[][], OptUncmin_methods minclass,
        ...
        slp = OptBlas_f77.ddot_f77(n, g, 1, sc, 1);
